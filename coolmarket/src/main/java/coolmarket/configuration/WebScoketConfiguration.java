...
public class WebScoketConfiguration implements WebSocketMessageBrokerConfigurer {
	...
	public void registerStompEndpoints(StompEndpointRegistry registry) {
		registry.addEndpoint("/chat").setAllowedOrigins("http://localhost:8080") // "*"로 설정시 cors 해제 왜 안됨 ?
				.withSockJS();
	...
	// 그럼 기본 url을 여기서 지정하고 컨트롤러에서 mapping주소를 다르게해서 여러 컨트롤러를 만드는건가.. 컨트롤러 여러개 만들어서
	// 어따쓰지 ..?
	@Override
	public void configureMessageBroker(MessageBrokerRegistry registry) {
		// publish, destination(목적지)이 /pub으로 된 메세지를 Controller의 MessageMapping으로 라우팅한다
		// 현재 컨트롤러의 MessageMapping은 /receive, 클라이언트에서 stompClient.send로 보낼때 주소가
		// /pub/receive가 됨
